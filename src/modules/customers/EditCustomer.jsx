    ...
    const load = async () => {
      try {
        ...
        form.setFieldsValue({
          ...
          gstDocUrlPage3: found.gstDocUrlPage3 || "",
  ...
  const handleSaveOnly = () => {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        setSaving(false);
        Modal.confirm({
          title: changes.length > 0 ? "Confirm Update & Sync" : "Save Customer Profile?",
          content: changes.length > 0 ? (
            <div className="space-y-3 font-sans mt-2">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {changes.map((c, idx) => (
                      <tr key={idx} className="hover:bg-slate-50/80 transition-colors">
                        ...
                        <td className="px-3 py-2 text-slate-400 line-through decoration-red-300">{c.oldVal}</td>
              ...
              <p className="text-xs text-slate-400">Proceed with sync?</p>
            </div>
          ) : (
