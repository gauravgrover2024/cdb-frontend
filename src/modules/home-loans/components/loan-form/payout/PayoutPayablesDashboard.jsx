...
const PayoutPayablesDashboard = () => {
  ...
  const loadPayables = async () => {
    try {
      ...
      // 1. Fetch Loans
      while (hasMore) {
        const res = await homeLoansApi.getAll({
          limit: pageSize,
          skip,
          noCount: true,
          sortBy: "leadDate",
  ...
  const updatePayableInBackend = async (payoutId, patch, activityAction = null) => {
    ...
    try {
      await homeLoansApi.update(targetLoan._id || targetLoan.loanId || targetLoan.id, {
        [key]: updatedList,
      });
      await loadPayables();
    } catch (err) {
  ...
  const applyBulkUpdates = async (updateFactory) => {
    ...
    await Promise.all(
      Array.from(updatesByLoan.entries()).map(([loanId, payload]) =>
        homeLoansApi.update(loanId, { [payload.key]: payload.updated }),
      ),
    );
  };
