...
const PayoutReceivablesDashboard = () => {
    ...
    async (record) => {
      ...
      try {
        ...
        const [loanResult, paymentResult, doResult] = await Promise.allSettled([
          loanRef ? homeLoansApi.getById(loanRef) : Promise.resolve(null),
  ...
  const loadReceivables = async () => {
    try {
      ...
      try {
        const fastRes = await homeLoansApi.getCollectionsReceivables({
          limit: 12000,
          skip: 0,
        });
      ...
      if (!usedFastCollectionsEndpoint) {
        ...
        let hasMore = true;
        while (hasMore) {
          const res = await homeLoansApi.getAll({ limit: pageSize, skip, noCount: true });
  ...
  const updateReceivableInBackend = async (
    ...
    try {
      await homeLoansApi.updateCollectionReceivable(
        normalizedId,
        {
