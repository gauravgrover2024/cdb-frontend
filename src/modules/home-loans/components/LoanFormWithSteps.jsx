...
const LoanFormWithSteps = ({ mode, initialData }) => {
  ...
  const fetchLoanById = useCallback(async (loanId) => {
    ...
    try {
      const json = await homeLoansApi.getById(loanId);
      return json?.data || null;
    } catch (error) {
      const list = await homeLoansApi.getAll();
      ...
      if (matched?._id && matched._id !== loanId) {
        try {
          const full = await homeLoansApi.getById(matched._id);
  ...
  const createLoan = useCallback(async (payload) => {
    const json = await homeLoansApi.create(payload);
  ...
  const updateLoan = useCallback(async (loanId, payload) => {
    const json = await homeLoansApi.update(loanId, payload);
    return json?.data || null;
  }, []);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        await homeLoansApi.update(effectiveLoanId, docPatch);
        docAutoSaveSignatureRef.current = signature;
      } catch (error) {
  ...
  const handleSaveLoan = useCallback(
    async (
      ...
      try {
        ...
        if (navigateAfterSave && shouldExit && !silent) navigate("/home-loans");
        return true;
      } catch (e) {
  ...
  const handleDisburseLoan = async (bankId, disbursementDate, remarks = "") => {
    try {
      ...
      await homeLoansApi.disburse(loanId, {
