...
import { Menu, X, ChevronDown, User, Sun, Moon, LogOut, Settings, Bell } from "lucide-react";
...
const Header = () => {
  ...
  const navigationGroups = [
    ...
    },
    {
      ...
      children: [
        { label: "Vehicle Inventory", path: "/vehicles", desc: "Master vehicle records" },
    ...
    <>
      ...
      >
        <div className="max-w-[1440px] mx-auto px-4 md:px-8 h-full flex items-center justify-between">
          {/* Brand Logo */}
          <div className="flex items-center group cursor-pointer shrink-0" onClick={() => handleNavigation("/")}>
            <div className={`relative transition-all duration-500 will-change-transform group-hover:scale-105 ${scrolled ? 'h-9' : 'h-10'}`}>
              <img
                ...
                {group.children ? (
                  <>
                    <button
                      ...
                      `}
                    >
                      <span className="opacity-70 group-hover/nav:opacity-100 transition-opacity">{group.icon}</span>
                      {group.label}
                      <ChevronDown
                        size={14}
                    ...
                    <div className="absolute top-full left-1/2 -translate-x-1/2 pt-3 opacity-0 invisible translate-y-2 group-hover/nav:opacity-100 group-hover/nav:visible group-hover/nav:translate-y-0 transition-all duration-300 ease-[cubic-bezier(0.23,1,0.32,1)] z-50">
                      <div className="w-[280px] bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl border border-border/50 p-2 overflow-hidden ring-1 ring-black/5">
                        <div className="px-3 py-2 border-b border-border/40 mb-1">
                           <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/60">{group.label} Operations</span>
                        </div>
                        {group.children.map((child) => (
                          ...
                          >
                            <div className="flex items-center justify-between">
                              ...
                              <div className={`w-1.5 h-1.5 rounded-full bg-primary transition-all duration-300 ${isActive(child.path) ? 'opacity-100 scale-100' : 'opacity-0 scale-0 group-hover/item:opacity-40 group-hover/item:scale-100'}`} />
                            </div>
          ...
          <div className="flex items-center gap-2 sm:gap-4 shrink-0">
                ...
                items: [
                  {
                    ...
                    label: (
                      <div className="px-4 py-3 min-w-[200px] border-b border-border/40 mb-2">
                        <div className="text-[13px] font-black text-foreground uppercase tracking-tight">{userData?.name || "Administrator"}</div>
                        <div className="text-[11px] text-muted-foreground font-medium">{userData?.email || "system@acillp.com"}</div>
            ...
            >
              <div className="flex items-center gap-2 cursor-pointer group/profile rounded-full pr-2 transition-all duration-300">
                ...
                <ChevronDown size={14} className="text-muted-foreground group-hover/profile:text-foreground transition-colors hidden sm:block" />
              </div>
            </Dropdown>
                  ...
                  <div key={idx} className="space-y-1">
                    {group.children ? (
                      ...
                      <button
                        ...
                        `}
                      >
                        <span className={`opacity-60 ${isActive(group.path) ? 'text-white' : ''}`}>{group.icon}</span>
            ...
            <div className="p-6 border-t border-border/50 bg-white dark:bg-black/50">
              <div className="flex items-center gap-4 mb-6 px-1">
                 ...
                 <div className="flex flex-col overflow-hidden">
                    <span className="text-sm font-black text-foreground truncate">{userData?.name || "Staff User"}</span>
                    <span className="text-[11px] text-muted-foreground font-medium truncate opacity-70">Control Panel Operator</span>
                 </div>
              </div>
